...
- 快速面板中的账户摘要可直接打开完整账户页；悬浮球本身不显示余额等私人信息。
- 正式模型与网关 Key 接口接入后，Codex/Claude Code 配置必须先生成预览，确认后才会创建备份并写入；配置文件被其他程序修改时，旧预览会失效。当前 mock 阶段只检测配置并阻止写入真实文件。
...
AI 工具配置备份位于应用本地数据目录的 `cli-config-backups` 子目录，并与资料库导出相互独立。备份只接受应用生成的 ID，不允许从界面选择任意路径恢复。退出 netapi.cc 账户不会静默恢复或删除已应用的 Codex/Claude Code 配置。
